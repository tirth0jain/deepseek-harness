...
本部署实际服务的网关路由（CommandCode、OpenCode）根本不发布价格：它们的模型列表只回答 id、名称和上下文长度，别无其他。因此「为用量定价」需要两半——让部署能够声明目录没有携带的费率，以及一个把 token 桶折算成金额、并在没有费率时不凭空发明的折叠。
...
模型选择器与 composer 座位中的每一行还以 `$in / $out` 显示费率，这正是让顺序变得可读的原因：部署的 `models` 列表顺序就是选择器的顺序。

## Alternatives considered

**只在 `PiAiModelProfile` 上加价格字段。** 那样部署可以声明费率，但金额仍然没有通向任何界面的路径；`LlmModelCost` 这道接缝正是让同一个折叠服务 Chat、Trajectory 以及此后任何界面的东西。

**从 pi-ai 目录为网关模型推导费率。** `opencode-go` 的目录把 `deepseek-v4-flash` 标为 `$0.22/$0.66`，那样无需一行配置就能为每个自动刷新的模型显示金额。它被否决了：价格属于实际计费的端点，而不属于描述另一个端点的厂商目录，而且本部署的两条路由在若干处与它不一致。目录价格仍然是目录模型的来源，那才是它权威的地方。
