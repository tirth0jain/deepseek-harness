...
**What this does not fix.** Opening a Session in the browser **resumes its Agent**. `history.follow` hands any prepared observation to `agents.resolveObservedAgent`, which composes and resumes a live Agent, and nothing disposes that Agent afterwards — so every chat opened *for reading* keeps its whole event graph for the life of the process. That is the largest single contributor, and it is why the observation-cache bound above is a ceiling on browsing rather than on the working set: a chat stops being cold the moment it is opened. The remaining copy is the live `Session.log` itself. Nothing releases an opened Session — the agent is created inside a service-owned effect and `AgentHandle.dispose` has no production caller — and that is the next thing to take. The container is also unsupervised: there is no unit for the harness, `memory.max` is unlimited, and the process reports no heap or RSS figure anywhere, so this was invisible until it fell over.
...
- **An opened Session is never released, and opening one to read it resumes its Agent.** Its parsed log is held for the lifetime of the harness process — there is no close, detach or release RPC, and no production caller of `AgentHandle.dispose` — so resident memory grows with the Sessions opened since the last restart and never comes back. Measured at 1.32 GB heap / 1.93 GB RSS for the 139k-record Session above. `/compact` does not help: compaction's own contract keeps the shadowed content in the session log, and the log is what is retained. The handoff memo is now byte-bounded and the prepared-observation cache is now `preparedSessionCacheSize`, which between them remove the extra copies; neither releases the graph, because `history.follow` promotes a prepared Session by resuming its Agent the moment the chat is opened. Until that promotion is made lazy, the working set is set by how many chats have been opened since the last restart.
