...
- **Search with ripgrep, not grep.** Use `rg` for all repo searches; add `--no-ignore` when built output (`lib/`, `dist/`, `.tsbuildinfo`, `node_modules`) must be included (diagnosing build artifacts). Plain `rg` respects `.gitignore` and is ~10-40× faster than `grep -r` on this tree (75K files, only ~12K source). Prefer `--type ts --type tsx` or path-scoped `rg <pat> packages/...` over `--type ts` alone (React files are `.tsx`). Note: `-uu` additionally searches hidden files (`.agents/`, `.git`), and `-uuu` also binary files — use deliberately, not by default.
