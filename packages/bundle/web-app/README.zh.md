...
运行 `dsh --profile web`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。启动时会打印带 token 的 URL；浏览器用它换取签名 cookie，再重定向到干净的根地址。你可以更改端口、关闭浏览器交接、允许额外主机，也可以绑定所有网络接口。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
大多数用户不需要设置这些；命令行 flag 会提供给下面四个设置——`--host`、`--port` 与 `--trusted-host` 来自本次调用，`--no-open` 仅对本次调用关闭浏览器交接，`--no-browser-auth` 则跳过启动令牌握手：
...
同一道围栏也决定浏览器能否编辑设置。若页面自身的 authority 既不是 loopback 也不在可信主机之列，设置文档就是只读的，因为浏览器半边对自身页面位置会得出相同结论。页面本身仍能加载——静态资源是公开的——但每个 `/api` 请求都会被 403 拒绝，因此账户卡片与模型目录都会是空的。请声明你实际访问所用的 authority：例如 `codeserver` 这样的 DNS 别名，或反向代理转发的宿主名。起作用的是地址栏里的名字，而不是它解析到的地址。

`--no-browser-auth` 为那些由上游负责认证访问者的部署去掉令牌交换。围栏仍然生效，因此仍需声明代理转发的 authority；此后凡是围栏放行的请求都被授权，打印出的 URL 不再携带令牌，也不会创建浏览器会话签名密钥。可达性由此成为全部访问策略，因此只应在绑定地址与所声明的 authority 都由你掌控时使用。
...
- **绑定所有网络接口会暴露到 LAN**——`--host 0.0.0.0` 现在可用于 LAN 与反向代理部署；启动时会采样并打印 LAN URL，且 /api 浏览器信任围栏仍要求 loopback、推导出的 LAN 或显式声明的 `--trusted-host` 主机，因此当浏览器通过不同主机名访问服务器时请配合使用 `--trusted-host`。
