...
运行 `dsh --profile web`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。该命令会打印干净的启动 URL——没有 token、没有 cookie、无需登录。你可以从命令行更改端口、关闭浏览器交接、允许额外主机，并可以绑定所有网络接口（`--host 0.0.0.0`）供局域网访问。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
启动后你会看到 `dsh web:` 行，其根 URL 就是普通的服务 URL。除非 `--no-open` 或 SSH 会话抑制，否则默认浏览器会直接打开该 URL——没有 token 交换、没有 cookie、没有重定向。页面加载且你可以与 agent（智能体）对话，就说明成功了。两种可预期的失败：前端未构建时，启动会以构建提示停止（checkout 中运行 `pnpm run build`）；浏览器无法打开时，stderr 会打印不含凭据的诊断，但服务器会继续运行——请自行打开已打印的启动 URL。
...
默认情况下 GUI 只接受本机的连接。绑定所有网络接口（`--host 0.0.0.0`）的部署也会允许 LAN 内的浏览器访问，此时打印的 URL 会附带一个 LAN 地址；`--trusted-host` 在两种情况下都能添加额外主机。Host 与 Origin 检查是唯一门槛：本 fork 已禁用浏览器认证，因此任何能访问该地址的人都能使用 GUI。LAN 地址只在启动时采样一次，因此之后的网络变化不会被感知——重启 GUI 以重新公告。
...
URL 行与浏览器交接都是就绪信号：监督方一观察到该行就发起 RPC，浏览器一打开就请求页面，因此两者只在 Loader 配置树结算且服务器开始服务后运行——在没有 Loader 的手工构建树中则立即运行。启动中途被释放的树不会宣告任何内容。
...
- **无浏览器认证**——本 fork 仅以 Host/Origin 防线提供服务；网络可达（`0.0.0.0`）的部署没有登录门槛，请务必放在可信网络中。
