...
function requestRoute(entry: {
  request?: { providerMetadata?: { provider?: string; model?: string } } | undefined
  node?: { providerMetadata?: { provider?: string; model?: string } } | undefined
}): { provider: string; model: string } | undefined {
  const provider = entry.request?.providerMetadata?.provider ?? entry.node?.providerMetadata?.provider
  const model = entry.request?.providerMetadata?.model ?? entry.node?.providerMetadata?.model
