...
根路径与配置的 index 响应会在读取 HTML 前调用 `ctx.connection.authorizeIndex`。在本 fork 中浏览器认证已禁用，因此该检查是无操作：所有 index 请求都会被服务，根 URL 无需任何 token。非 index 文件仍是公开静态资源。该 index 检查的形状归 Connection 所有。
