...
| `cost` (per model) | catalog price | Published list rate in USD per million tokens; stated per model, it prices that route's usage estimates |
...
A model entry's `cost` block states a published list rate — `input` and `output` are required together (a rate naming only one would bill the other at nothing), `cacheRead` and `cacheWrite` are optional and default to zero. It is a display rate: the harness estimates spend from it in the Chat usage dialog and the Trajectory inspector, and a model listing endpoint never publishes prices, so a gateway route carries whatever the deployment states. A model with no rate reports no amount rather than `$0.00`, which is why an unpriced route shows nothing instead of a free one. `reasoningEfforts` declares a model's selectable thinking levels: each key is a level selectors offer, its value the spelling dispatch sends on the wire, so `max: ultra` renames a level for a gateway with its own vocabulary. Omitting the field keeps the installed catalog entry's capability; `false` declares a non-reasoning model. `compat` switches reshape the request for endpoints pi-ai cannot recognize — which role carries the system prompt, which field caps output, how a thinking level travels — configurable per route and per model. A model neither the entry nor the installed catalog sizes takes the route's `defaultContextWindow` and `defaultMaxTokens` fallbacks.
